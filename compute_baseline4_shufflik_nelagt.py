...
from utils import compute_F, compute_mtt_abs, torch_delete, \
     chunks, select_permitted_rewirings_shufflik, compute_mean_expected_path, get_index, get_row_diff, vectorized_get_row_diff
...
with open(f'/home/ubuntu/phd/reachability/data/{dataset}/similarity_matrices/similarity_matrix_{newspaper}_from_{start_date}_to_{end_date}','rb') as f:
...
for threshold in [0.95]:
    with open(f'/home/ubuntu/phd/reachability/data/{dataset}/transition_probabilities/transition_probabilities_{newspaper}_from_{start_date}_to_{end_date}_k_{k}','rb') as f:
        trans_prob = torch.tensor(pickle.load(f), dtype=precision, device=torch.device('cpu'))

    # compute all possible edge rewirings (combination of edge swaps within the same source node)
    permitted_rewirings = select_permitted_rewirings_shufflik(trans_prob)
    print(f'Number of permitted rewirings = {len(permitted_rewirings)}')
    ...
    updated_trans_prob = updated_trans_prob.to(device)
    # free space
    ...
    for m in range(int(n_rewirings/batch_size)):
        for split in permitted_rewirings_chunked:
            ...
            # Save current state
            with open(f'results/Shufflik/rewirings/accepted_rewirings_{newspaper}_from_{start_date}_to_{end_date}_k_{k}_Shufflik_batch_size_{batch_size}_damping_factor_{damping_factor}', 'wb') as f:
